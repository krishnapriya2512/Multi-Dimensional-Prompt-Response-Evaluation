...
st.title("Multi-Dimensional Prompt Response Evaluation")

st.set_page_config(page_title="Multi-Dimensional Prompt Evaluation", layout = "wide")
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["Analysis Dashboard","Prompt Response Evaluator", "GPT vs ML", "Bias and Toxicity", "Image Generator", "Resume Evaluator"])
# ------------------------------------Analysis Dashboard -------------------------------------------------------
...
with tab1:
    ...
    # ==============GPT Vs Traditional Model ===============
    ...
    elif section == "GPT vs Traditional Model":
        st.header("GPT vs Traditional Model Dashboard")

        # Dashboard Discription
        st.markdown("""
        This dashboard compares the performance of **Large Language Models (LLMs)** (e.g., GPT) against **traditional NLP models** across a variety of text-based tasks.
        #### Tasks Evaluated
        - **Summarization** – Generating concise summaries from long text  
        - **Sentiment Analysis** – Identifying polarity of the text  
                        
        #### Models Compared
        - **GPT (LLM-based approaches)** → distilbert-base-uncased-finetuned-sst-2-english, facebook/bart-large-cnn
        - **Traditional Models** → TF-IDF, Logistic Regression, Naïve Bayes, TextRankSummarizer

        #### Evaluation Metrics

        To assess model performance, both **objective metrics** and **qualitative evaluations** were used:  

        - **TF-IDF Similarity**: Measures surface-level word overlap between article and summary.  
        - **Semantic Similarity**: Captures meaning preservation using embeddings.  
        - **Lexical Diversity**: Evaluates vocabulary richness in the summary.  
        - **Compression Ratio**: Assesses how much the summary condenses the original text.  
        - **Article Sentiment & Summary Sentiment**: Compares emotional tone between article and summary.  
        - **Repetition Score**: Detects redundant content in the summary.  
        - **Readability Score**: Estimates how easy the summary is to read (scaled to 100).

        #### Goal of the Dashboard
        This dashboard allows users to:  

        - Compare **GPT vs traditional models** across tasks  
        - Explore trade-offs between **accuracy, quality, and latency**  
        - Understand **strengths of LLMs vs efficiency of traditional models**  
        - Gain insights into **when to use LLMs vs traditional NLP approaches**  
        """)
        ...
        # ===== Traditional Model =====
        with col1:
            st.markdown("###### Traditional Model (Logistic Regression)")

            # Create figure for consistent sizing
            fig, ax = plt.subplots(figsize=(6, 5))
            cm = confusion_matrix(sentiment_tra['y_test'], sentiment_tra['y_pred'])
            
            # Seaborn heatmap
            sns.heatmap(cm, annot=True, fmt='d', cmap='Wistia', cbar=False, ax=ax)
            ...
            plot_f1_per_class(sentiment_tra['y_test'], sentiment_tra['y_pred'], 
                                        "Traditional Model Classification Report", categories= ["Negative","Positive"], figsize=(16,8))


        # ===== GPT Model =====
        with col2:
            st.markdown("###### GPT Model (BERT)")

            # Create figure for consistent sizing
            fig, ax = plt.subplots(figsize=(6, 5))
            cm = confusion_matrix(sentiment_gpt['label'], sentiment_gpt['sentiments_label'])
            ...
            # Classification report
            plot_f1_per_class(sentiment_gpt['label'], sentiment_gpt['sentiments_label'], 
                                        "Transformer Model Classification Report",categories= ["Negative","Positive"], figsize=(16,8))
        
        # ===Accuracy score======
        accuracy_scores = {
            "Traditional Model": accuracy_score(
                sentiment_tra['y_test'], sentiment_tra['y_pred']
            ),
            "Transformer Model": accuracy_score(
                sentiment_gpt['label'], sentiment_gpt['sentiments_label']
            )
        }
        ...
        colors = plt.cm.Wistia(np.linspace(0, 1, len(models)))
        ...
        # ============ Summarisation==================
        st.markdown("##### Summarisation Model Comparison")

        metric_cols = [
            'tfidf_similarity', 'semantic_similarity', 'lexical_diversity',
            'compression_ratio', 'article_sentiment', 'summary_sentiment',
            'repetition_score', 'readability_score'
        ]

        # Melt the DataFrame for plotting
        melted_df = summarise_df.melt(id_vars='model_used', value_vars=metric_cols,
            var_name='Metric', value_name='Score'
        )

        # Create figure & plot
        fig, ax = plt.subplots(figsize=(16, 8))
        sns.barplot(data=melted_df, x='Metric', y='Score', hue='model_used', palette='Wistia', ax=ax)
        ...
        ax.set_title('Evaluation Metrics Comparison by Model Type', fontsize = 16)
        plt.tight_layout()
...
#-------------------------------------------------------
# Caching Model loads for performance in Streamlit
#---------------------------------------------------------
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

@st.cache_resource
def load_sklearn_sentiment():
    models_path = os.path.join(BASE_DIR, "models")
    lr_path = os.path.join(models_path, "logistic_clf.joblib")
    vect_path = os.path.join(models_path, "tfidf_vectorizer.joblib")

    if os.path.exists(lr_path) and os.path.exists(vect_path):
        clf = joblib.load(lr_path)
        vect = joblib.load(vect_path)
        return clf, vect
    else:
        st.error(f"Model files not found in {models_path}")
        return None, None


@st.cache_resource
def load_transformer_sentiment(model_path_or_name: str="distilbert-base-uncased-finetuned-sst-2-english"):
    try:
        nlp = pipeline("sentiment-analysis", model=model_path_or_name, tokenizer=model_path_or_name)
        return nlp
    except Exception as e:
        st.warning(f"Transformer sentiment load failed: {e}")
        return None
    
@st.cache_resource
def load_summarizer(model_path_or_name: str="facebook/bart-large-cnn"):
    try:
        summarizer = pipeline("summarization", model=model_path_or_name, tokenizer=model_path_or_name)
        return summarizer
    except Exception as e:
        st.warning(f"Summarizer load failed: {e}")
        return None

#--------------------------------------------------
# Utility functions for Traditional vs GPT models
#-----------------------------------------------------
...
def extract_text_from_url(url: str) -> str:
    try:
        ...
        st.warning(f"Failed to extract text from URL: {e}")
        return ""
    
# Get Extractive Summary
def extractive_summary(text: str, num_sentences: int = 3) -> str:
    # Simple summariser using LexRank (sumy)
    ...
    try: 
        ...
        return f"[extractive summarizer error] {e}"
    
# computing Rogue Scores
def compute_rouge_scores(hypothesis: str, reference: str) -> Dict[str, float]:
    scorer = rouge_scorer.RougeScorer(['rouge1', 'rouge2', 'rougeL'], use_stemmer=True)
    try:
        scores = scorer.score(reference, hypothesis)
        # return fmeasure of each
        return {k: v.fmeasure for k, v in scores.items()}
    except Exception as e:
        return {"rouge1": 0.0, "rouge2": 0.0, "rougeL": 0.0}

# Normalising the output labels to POS/NEG  
def sentiment_label_map_from_transformer(label:str) -> str:
    lab = label.lower()
    if "pos" in lab:
        return "POS"
    if "neg" in lab:
...
# Compare Traditional and GPT model
def compare_models(metrics_Tra: Dict[str, float], metrics_gpt: Dict[str, float], metric_name: str) -> str:
    """Compare two models based on their metrics."""
    tra = metrics_Tra.get(metric_name, 0)
    gpt = metrics_gpt.get(metric_name, 0)
...
#--------------------------------------------------
with tab3:
    st.title("Traditional VS GPT Models - Sentiment analysis and Text Summarization")
    st.markdown("Compare Traditional and transformer based model to check which has better performance for task.")
...
#----------------------------------------------
    ...
    with left_col:
        ...
        input_text = st.text_area("enter text or Sentence to analyse sentiment:", height=180)

        st.subheader("Models to run")
        run_lr = st.checkbox("Traditional (Logistic Regression)", value=True)
        run_transformer = st.checkbox("Transformer Based Model(BERT)", value=True)
        run_gpt = st.checkbox("GPT via Gemini", value=True)

        models_folder = os.path.join(BASE_DIR, "models")

        # load models (deferred)
        lr_clf, lr_vect = (None, None)
        if run_lr:
            lr_clf, lr_vect = load_sklearn_sentiment()
            if lr_clf is None or lr_vect is None:
                st.write("Sklearn model or vectorizer not found in models folder. Export them in your notebook as joblib files and place them in ./models.")

        transformer_nlp = None
        if run_transformer: 
            # try loading from local models folder first
            local_tf_path = os.path.join(models_folder, "transformer_sentiment")
            if os.path.exists(local_tf_path):
                transformer_nlp = load_transformer_sentiment(local_tf_path)
            else:
                transformer_nlp = load_transformer_sentiment()
        ...
        latency_data = []
        if st.button("Run Sentiment Models"):
            if not input_text.strip():
                ...
                with cols[0]:
                    ...
                    if run_lr and lr_clf is not None and lr_vect is not None:
                        start_time = time.time()
                        X = lr_vect.transform([input_text])
                        proba = lr_clf.predict_proba(X)[0]
                        try:
                            classes = lr_clf.classes_
                            class_proba = {str(c): round(float(p), 4) for c, p in zip(classes, proba)}
                        except Exception:
                            class_proba = {"NEG": round(float(proba[0]), 4), "POS": round(float(proba[1]), 4)}
                        pred = lr_clf.predict(X)[0]
                        pred_value = "POS" if str(pred) == "1" else "NEG" if str(pred) == "0" else pred
                        st.write(f"**Prediction:** {pred_value}")
                        st.write(f"**Class Probabilities:** \n {class_proba}")
                        elapsed = round(time.time() - start_time, 3)
                        latency_data.append({"Model": "Traditional Model", "Latency (seconds)": elapsed})
                    else:
                        st.write("Traditional sentiment model not available.")

                # Transformer Model
                with cols[1]:
                    st.markdown("##### Transformer Model")
                    if run_transformer and transformer_nlp is not None:
                        start_time = time.time()
                        res = transformer_nlp(input_text)[0]
                        label = sentiment_label_map_from_transformer(res.get("label", ""))
                        score = res.get("score", None)
                        st.write(f"**Prediction**: {label}")
                        st.write(f"**Probability**: {round(float(score), 4) if score else 'N/A'}")
                        elapsed = round(time.time() - start_time, 3)
                        latency_data.append({"Model": "Transformer Model", "Latency (seconds)": elapsed})
                    else:
                        st.write("Transformer sentiment model not available.")

                # Gemini API
                with cols[2]:
                    st.markdown("##### Gemini API")
                    if run_gpt and model is not None:
                        start_time = time.time()
                        prompt_sentiment = f"""
        Classify the sentiment of the following text as POS or NEG. 
        ...
        **Probability**: 0.95
        """
                        response = get_gemini_response(prompt_sentiment, input_text)
                        st.write(response)
                        elapsed = round(time.time() - start_time, 3)
                        latency_data.append({"Model": "Gemini API", "Latency (seconds)": elapsed})
                    else:
                        st.write("Gemini API not available.")
        ...
        # ========== Evaluation upload for sentiment =========
        st.markdown("---")
        st.subheader("Evaluate Sentiment Models on test CSV")
        st.markdown("Upload a CSV with columns: text, label (POS/NEG or 1/0). The app will compute accuracy, precision, recall, f1.")
        uploaded_sent_csv = st.file_uploader("Upload sentiment test CSV", type=["csv"]) 
        if uploaded_sent_csv is not None:
            df_eval = pd.read_csv(uploaded_sent_csv)
            if "text" not in df_eval.columns or "label" not in df_eval.columns:
                st.error("CSV must have columns: text, label")
            else:
                ...
                df_eval["label_norm"] = df_eval["label"].apply(norm_label)
                ...
                if run_lr and lr_clf is not None and lr_vect is not None:
                    ...
                    X = lr_vect.transform(df_eval["text"].astype(str).tolist())
                    y_pred = lr_clf.predict(X)

                    # If your LR predicts "POS"/"NEG", convert them:
                    if isinstance(y_pred[0], (np.integer, int)):
                        y_pred_num = y_pred.astype(int)
                    else:
                        y_pred_num = [1 if str(p).upper() in ["POS", "1", "TRUE"] else 0 for p in y_pred]
                    
                results['Traditional'] = {
                    'accuracy': accuracy_score(df_eval["label_norm"], y_pred_num),
                    'precision': precision_score(df_eval["label_norm"], y_pred_num, zero_division=0),
                    'recall': recall_score(df_eval["label_norm"], y_pred_num, zero_division=0),
                    'f1': f1_score(df_eval["label_norm"], y_pred_num, zero_division=0)
                }   
                elapsed = round(time.time() - start_time, 3)
                latency_data.append({"Model": "Traditional Model", "Latency (seconds)": elapsed})
                ...
                if run_transformer and transformer_nlp is not None:
                    y_pred_num = []
                    for txt in df_eval['text'].astype(str).tolist():
                        out = transformer_nlp(txt)[0]
                        label = sentiment_label_map_from_transformer(out.get('label',''))
                        y_pred_num.append(1 if label.upper() == "POS" else 0)
                    results['GPT/Transformer'] = {
                        'accuracy': accuracy_score(df_eval["label_norm"], y_pred_num),
                        'precision': precision_score(df_eval["label_norm"], y_pred_num, zero_division=0),
                        'recall': recall_score(df_eval["label_norm"], y_pred_num, zero_division=0),
                        'f1': f1_score(df_eval["label_norm"], y_pred_num, zero_division=0)
                ...
                # decide winner based on f1
                if len(results) >= 2:
                    best = max(results.items(), key=lambda x: x[1]['f1'])[0]
                    st.success(f"Best sentiment model on this test set (by F1): {best}")
...
# -----------------------
    with right_col:
        ...
        # ------------------- Input type -------------------
        input_choice = st.radio("Input type", ["Text", "URL"], index=0)
        ...
        if input_choice == "Text":
            ...
            url = st.text_input("Enter the URL to summarise")
            if url and st.button("Fetch URL"):
                with st.spinner("Fetching article..."):
                    fetched_text = extract_text_from_url(url)
                    if fetched_text:
                        st.session_state.src_text = fetched_text
                        st.success("Fetched text (first 1000 chars):")
                        st.write(fetched_text[:1000] + ("..." if len(fetched_text) > 1000 else ""))
                    else:
                        st.error("Failed to fetch text from the provided URL.")

        # ------------------- Summarizer settings -------------------
        num_sent = st.slider(
        "Extractive summary: number of sentences (Traditional)",
        min_value=1, max_value=30, value=3
        )
        run_extractive = st.checkbox("Run Traditional extractive summarizer (LexRank)", value=True)
        run_transformer_summ = st.checkbox("Run Transformer/LLM summarizer", value=True)
        run_gemini = st.checkbox("Run Gemini API", value = True)
        ...
        def safe_chunks(text, max_words_per_chunk=200):
            """Split text into word-based chunks to avoid tokenizer index errors."""
            words = text.split()
            chunk = []
            for w in words:
                chunk.append(w)
        ...
        if st.button("Summarize"):
            if not st.session_state.src_text.strip():
                st.warning("No text to summarise. Paste text or fetch from URL first.")
            else:
                clean_text = st.session_state.src_text.strip()

                # -------- Traditional Extractive --------
                st.subheader("Traditional Extractive Summary")
                if run_extractive:
                    start_time = time.time()
                    try:
                        trad_summary = extractive_summary(clean_text, num_sentences=num_sent)  # user-defined
                        st.write(trad_summary)
                    except Exception as e:
                        st.error(f"Traditional extractive summary error: {e}")
                    elapsed = round(time.time() - start_time, 3)
                    latency_data.append({"Model": "Traditional Model", "Latency (seconds)": elapsed})
                else:
                ...
                # -------- Transformer / GPT --------
                st.subheader("Transformer / GPT Summary")
                if run_transformer_summ and summarizer_pipe is not None:
                    start_time = time.time()
                    summaries = []
                    try:
                        for chunk in safe_chunks(clean_text, max_words_per_chunk=200):
                            if len(chunk.split()) < 10:
                                continue  # skip very short chunks
                            out = summarizer_pipe(chunk, max_length=160, min_length=30, do_sample=False)
                            if out and "summary_text" in out[0]:
                        ...
                        if summaries:
                            ...
                            st.error("No valid summaries could be generated.")
                    except Exception as e:
                        st.error(f"Transformer summarizer error: {e}")

                    elapsed = round(time.time() - start_time, 3)
                    latency_data.append({"Model": "Transformer Model", "Latency (seconds)": elapsed})
                else:
                    st.write("Transformer summarizer not available. Check model name or connectivity.")

                # -------- Gemini API --------
                if run_gemini:
                    start_time = time.time()
                    summarise_prompt = """
        Role: You are an advanced text summarization system.
        Objective: Summarize the given text into a clear, concise version that preserves the core meaning, key facts, and important details, while removing redundancy and irrelevant information.
        Guidelines:
        - Capture the main idea and most important supporting points.
        - Avoid adding new information or personal opinions.
        - Use clear and simple language.
        - Maintain neutrality and objectivity.
        - Adjust the summary length based on instructions (short, medium, or detailed). If no length is specified, keep it concise (3–5 sentences).
        Output Format:
        [Provide the summarized text]
                    """
                    try:
                        gemini_response = get_gemini_response(clean_text, summarise_prompt)
                        st.subheader("**Gemini API**", gemini_response)
                    except Exception as e:
        ...
        # Evaluation upload for summarization
        st.markdown("---")
        st.subheader("Evaluate summarizers on a test CSV")
        st.markdown("Upload CSV with columns: source_text, reference_summary")
        uploaded_sum_csv = st.file_uploader("Upload summarization test CSV", type=["csv"], key="sum_csv")

        if uploaded_sum_csv is not None:
            df_sum = pd.read_csv(uploaded_sum_csv)
            if "source_text" not in df_sum.columns or "reference_summary" not in df_sum.columns:
                st.error("CSV must contain source_text and reference_summary")
            else:
                rouge_results = {"Traditional": {"rouge1": [], "rouge2": [], "rougeL": []},
                                "GPT/Transformer": {"rouge1": [], "rouge2": [], "rougeL": []}}

                for _, row in df_sum.iterrows():
                    src = str(row['source_text'])
                    ref = str(row['reference_summary'])

                    # Traditional
                    trad = extractive_summary(src, num_sentences=num_sent) if run_extractive else ""
                    trad_scores = compute_rouge_scores(trad, ref)
                    for k in trad_scores:
                        rouge_results['Traditional'][k].append(trad_scores[k])

                    # Transformer
                    if run_transformer_summ and summarizer_pipe is not None:
                        try:
                            out = summarizer_pipe(src, max_length=160, min_length=30, do_sample=False)
                            trans_sum = out[0]['summary_text']
                        except Exception:
                            trans_sum = ""
                    else:
                        trans_sum = ""
                    trans_scores = compute_rouge_scores(trans_sum, ref)
                    for k in trans_scores:
                        rouge_results['GPT/Transformer'][k].append(trans_scores[k])

                # Aggregate metrics
                agg = {}
                for model in rouge_results:
                    agg[model] = {k: float(np.mean(rouge_results[model][k])) if rouge_results[model][k] else 0.0
                                for k in rouge_results[model]}

                st.subheader("ROUGE metrics (averaged)")
                df_metrics = pd.DataFrame(agg).T
                st.write(df_metrics)

                # Pick winner by rougeL
                winner = compare_models(agg.get('Traditional', {}), agg.get('GPT/Transformer', {}), metric_name='rougeL')
                st.success(f"Best summarizer on this dataset (by rougeL): {winner}")
...
with tab6:
    ...
    # Function to get PDF file extraction to image
    ...
    #st.set_page_config(page_title="Resume Evaluator using LLM")
    ...
    if submit1:
        ...
        if uploaded_file is not None:
            ...
            end_time = time.time()
            latency = round(end_time - start_time, 3)
            st.subheader("Resume Evaluation Metrics")
